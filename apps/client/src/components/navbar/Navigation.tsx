...
const Navigation = () => {
  ...
  return (
    ...
    //     </div>
    //   </div>
    <div className="max-lg:hidden flex h-full w-max items-center gap-6 bottom-3 whitespace-nowrap">
      {/* <div className=" w-max ">
        <Logo sm />
      </div> */}
      ...
      <div className=" rounded-md">
        <Link
          to={'/'}
          className={`relative text-sm pb-6 hover:font-semibold transition-all duration-200 font-light
	${(!currentPath || currentPath === 'following') && ' font-semibold'}
  `}
        >
          <span className="">For You</span>
          {/* {(!currentPath || currentPath === 'following') && (
            <div className="absolute top-5 left-1/2 w-[2px] h-[2px] "></div>
          )} */}
        </Link>
      </div>
    </div>
